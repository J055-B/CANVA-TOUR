...
// Team Targets — one extra column vs the main Tour de Callisto's version:
// MONTHLY TARGET (the Target sheet's "Targ" column, e.g. 147 for Canva),
// inserted between DAILY TARGET and % OF TARGET.
const TARGETS_GRID_COLS = '56px 1.1fr 0.6fr 0.8fr 0.8fr 0.85fr 0.9fr 1fr'
...
export default function DetailedLeaderboard({ entries }: { entries: LeaderboardEntry[] }) {
  return (
    <div>
      <SectionHeader icon={<Trophy size={16} color="#FFD400" />} accent="#FFD400" title="LEADERBOARD" subtitle="Live team standings, updated in real time" />
      ...
      <div>
        <SectionHeader icon={<Target size={16} color="#2DD4BF" />} accent="#2DD4BF" title="TEAM TARGETS" subtitle="Daily, monthly and weekly CANVA targets" />
        <div className="overflow-x-auto">
          <div className="min-w-[760px]">
            ...
            <div className="mt-3 space-y-2">
              {entries.map((e, i) => {
                ...
                // from the main Tour, same fix applied here).
                ...
                return (
                  ...
                  >
                    ...
                    <div>{e.monthlyTarget ? e.monthlyTarget.toLocaleString() : '—'}</div>
                    <div className="font-semibold" style={{ color: progressColor(e.targetPct, true) }}>
                      {e.targetPct.toFixed(1)}%
                    </div>
